...
@Component({
  ...
  templateUrl: './aceh-details.page.html',
  styleUrls: ['./aceh-details.page.scss'],
})
export class AcehDetailsPage implements OnInit {
  ...
  ngOnInit() {
    from(this.presentLoading()).subscribe(() => {
      this.dataService.getAcehDescription().subscribe(descriptions => this.description$ = descriptions, (error) => this.presentAlertInternet(), () => this.loadingController.dismiss());
    });
  }
