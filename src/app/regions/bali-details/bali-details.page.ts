...
@Component({
  ...
  templateUrl: './bali-details.page.html',
  styleUrls: ['./bali-details.page.scss'],
})

export class BaliDetailsPage implements OnInit {
  ...
  ngOnInit() {
    from(this.presentLoading()).subscribe(() => {
      this.dataService.getBaliDescription().subscribe(descriptions => this.description$ = descriptions, (error) => this.presentAlertInternet(), () => this.loadingController.dismiss());
    });
  }
