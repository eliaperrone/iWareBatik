...
@Component({
  ...
  templateUrl: './sumbar-details.page.html',
  styleUrls: ['./sumbar-details.page.scss'],
})
export class SumbarDetailsPage implements OnInit {
  ...
  ngOnInit() {
    from(this.presentLoading()).subscribe(() => {
      this.dataService.getWestSumatraDescription().subscribe(descriptions => this.description$ = descriptions, (error) => this.presentAlertInternet(), () => this.loadingController.dismiss());
    });
  }
