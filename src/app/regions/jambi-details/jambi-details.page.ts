...
@Component({
  ...
  templateUrl: './jambi-details.page.html',
  styleUrls: ['./jambi-details.page.scss'],
})
export class JambiDetailsPage implements OnInit {
  ...
  ngOnInit() {
    from(this.presentLoading()).subscribe(() => {
      this.dataService.getJambiDescription().subscribe(descriptions => this.description$ = descriptions, (error) => this.presentAlertInternet(), () => this.loadingController.dismiss());
    });
  }
