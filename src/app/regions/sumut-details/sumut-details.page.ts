...
@Component({
  ...
  templateUrl: './sumut-details.page.html',
  styleUrls: ['./sumut-details.page.scss'],
})
export class SumutDetailsPage implements OnInit {
  ...
  ngOnInit() {
    from(this.presentLoading()).subscribe(() => {
      this.dataService.getNorthSumatraDescription().subscribe(descriptions => this.description$ = descriptions, (error) => this.presentAlertInternet(), () => this.loadingController.dismiss());
    });
  }
